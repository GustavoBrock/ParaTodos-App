...
import bcrypt # A ser usado nas funções de login/cadastro

# O ID da sua planilha, lido do secrets.toml (agora como variável de ambiente)
SHEET_ID = st.secrets.get("SHEET_ID", None)

# --- NOVO BLOCO DE AUTENTICAÇÃO E CRIAÇÃO DO ARQUIVO DE CREDENCIAIS ---
# Crie um dicionário com as credenciais lidas dos secrets do Streamlit
try:
    # Acessa os segredos JSON para a autenticação do gspread
    creds_json = st.secrets.get("gspread_creds")

    if not creds_json:
        # Se as credenciais gspread não forem encontradas, tenta a abordagem antiga de TOML
        creds_json = st.secrets.get("gspread")
        if creds_json:
            creds_json = creds_json.to_dict()

    if creds_json:
        # Salva o JSON em um arquivo temporário no ambiente do Streamlit Cloud
        ...
        st.error("Credenciais do Google Sheets não encontradas. Verifique seu secrets.toml.")
        st.stop()

except Exception as e:
    st.error(f"Erro ao carregar as credenciais. Verifique o formato no secrets.toml. Erro: {e}")
    st.stop()
